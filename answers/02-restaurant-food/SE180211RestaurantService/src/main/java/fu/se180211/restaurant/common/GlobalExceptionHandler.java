...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponseDTO<Void>> handleBusiness(BusinessException ex) {
        return ResponseEntity.status(ex.getHttpStatus())
                .body(ApiResponseDTO.error(ex.getApiStatus(), ex.getMessage()));
    }
    ...
    public ResponseEntity<ApiResponseDTO<Void>> handleValidation(Exception ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ApiResponseDTO.error(ResponseStatuses.VALIDATION_FAILED, "Data validation failed"));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponseDTO<Void>> handleOther(Exception ex) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiResponseDTO.error(ResponseStatuses.INTERNAL_ERROR, "Internal server error"));
    }
}
